...
def focal_loss(
    ...
    log_input_soft: Tensor = input.log_softmax(dim=1)
...
def binary_focal_loss_with_logits(
    ...
    assert input.shape[0] == target.shape[0], f'Expected input batch_size ({input.shape[0]}) to match target batch_size ({target.shape[0]}).'

    if pos_weight is None:
        pos_weight = torch.ones([input.shape[1], *([1] * len(input.shape[2:]))], device=input.device, dtype=input.dtype)

    assert isinstance(pos_weight, Tensor), "Positive weights should be a Tensor"
...
def dice_loss(y_real, y_pred):
    ...
    return 1-torch.mean(2*y_real*y_pred_class+1)/(torch.mean(y_real+y_pred_class)+1)